...
- **管理员总览**：支持今日、近 7 天、近 30 天和自定义日期范围，可查看全部 Key 或按 Key owner 筛选，并提供用量、成本、延迟、分布和认证账户信息。部分计价时仅展示已知成本小计；全部已用模型均未计价时隐藏成本入口与总览成本卡。账户一级列表会批量刷新用量、额度和健康指标，点击后进入单账户详情。
...
- **安全连接**：支持 Keeper 管理员密码或 API Key（sk）登录、HTTP/SOCKS5 代理；请求使用 `Keeper-UsagePanel_0.5.1 admin` 或 `Keeper-UsagePanel_0.5.1 sk` 标识会话，未改变连接参数时保存设置不会重复登录。凭据和代理认证信息使用 Windows DPAPI 加密保存在当前用户配置中，连接失败时可复制脱敏 ERRLOG 排查代理、DNS、TLS 与超时问题。受信任内网代理使用未知 CA 时可显式关闭 HTTPS 证书验证，但建议优先安装内网 CA。
...
管理员可在一级列表查看全部账户的累计用量汇总，以及各账户的额度与近 5 小时健康状态，再进入二级页面查看当前额度、额度历史、请求明细和错误事件。面板会聚合 Keeper 的全部账户页，并以一次批量请求读取已有额度缓存，不会触发上游额度刷新。
