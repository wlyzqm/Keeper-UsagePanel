...
- **管理员总览**：支持今日、近 7 天、近 30 天和自定义日期范围，可查看全部 Key 或按 Key owner 筛选，并提供用量、成本、延迟、分布和认证账户信息。部分计价时仅展示已知成本小计；全部已用模型均已计价时不显示额外提醒，全部已用模型均未计价时隐藏成本入口与总览成本卡。账户一级列表会批量刷新用量、剩余额度和健康指标，点击后进入单账户详情。
- **状态与主题**：以健康状态点和文字结论展示连接及请求状态，支持浅色、深色主题、预设或自定义主题色和自定义悬浮窗字体。
- **更新管理**：每次启动读取一次 GitHub Release 固定更新清单，也可在设置中手动检查；不使用受共享 IP 额度限制的 GitHub REST API。发现新版本时只在面板显示入口，用户点击后才展示更新说明和升级选项。支持跳过版本及一键静默更新，安装版静默覆盖，便携版原位替换并重启；检查与下载沿用 Keeper 连接的代理和证书设置，安装前校验 SHA-256。
- **安全连接**：支持 Keeper 管理员密码或 API Key（sk）登录、HTTP/SOCKS5 代理；请求使用 `Keeper-UsagePanel_0.5.5 admin` 或 `Keeper-UsagePanel_0.5.5 sk` 标识会话，未改变连接参数时保存设置不会重复登录。凭据和代理认证信息使用 Windows DPAPI 加密保存在当前用户配置中，连接失败时可复制脱敏 ERRLOG 排查代理、DNS、TLS 与超时问题。受信任内网代理使用未知 CA 时可显式关闭 HTTPS 证书验证，但建议优先安装内网 CA。
...
管理员可在一级列表查看全部账户的累计用量汇总，以及各账户的剩余额度与近 5 小时健康状态，再进入二级页面查看当前剩余额度、额度历史、请求明细和错误事件。面板会聚合 Keeper 的全部账户页，并以一次批量请求读取已有额度缓存，不会触发上游额度刷新。

![认证账户额度](docs/quota-light.png)

### 连接设置

设置通过连接参数、外观与样式、悬浮窗行为、版本与更新四个标签页分页管理，可配置 Keeper 地址、登录方式、代理、刷新间隔、展示/隐藏行为、主题色和字体，并可手动检查更新。管理员凭据、sk 与带认证信息的代理地址不会以明文保存。
